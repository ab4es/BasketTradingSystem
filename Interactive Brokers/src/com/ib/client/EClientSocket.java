...
public class EClientSocket {
	...
	protected synchronized void startAPI() {
		...
		try {
			...
			error(EClientErrors.NO_VALID_ID, EClientErrors.FAIL_SEND_STARTAPI,
					"" + e);
			close();
		}
	}
			...
			if (m_serverVersion >= MIN_SERVER_VER_LINKING) {
				...
				if (scannerSubscriptionOptionsCount > 0) {
					for (int i = 0; i < scannerSubscriptionOptionsCount; ++i) {
						TagValue tagValue = (TagValue) scannerSubscriptionOptions
								.get(i);
	...
	public synchronized void reqMktData(int tickerId, Contract contract,
		...
		try {
			...
			if (m_serverVersion >= MIN_SERVER_VER_LINKING) {
				...
				int mktDataOptionsCount = mktDataOptions == null ? 0
						: mktDataOptions.size();
	...
	public synchronized void reqHistoricalData(int tickerId, Contract contract,
		...
		try {
			...
			if (m_serverVersion >= MIN_SERVER_VER_LINKING) {
				...
				int chartOptionsCount = chartOptions == null ? 0 : chartOptions
						.size();
	...
	public synchronized void reqRealTimeBars(int tickerId, Contract contract,
			...
			Vector<TagValue> realTimeBarsOptions) {
		// not connected?
		if (!m_connected) {
			notConnected();
			return;
		...
		try {
			...
			if (m_serverVersion >= MIN_SERVER_VER_LINKING) {
				...
				int realTimeBarsOptionsCount = realTimeBarsOptions == null ? 0
						: realTimeBarsOptions.size();
				if (realTimeBarsOptionsCount > 0) {
					for (int i = 0; i < realTimeBarsOptionsCount; ++i) {
						TagValue tagValue = (TagValue) realTimeBarsOptions
								.get(i);
	...
	public synchronized void reqMktDepth(int tickerId, Contract contract,
			int numRows, Vector<TagValue> mktDepthOptions) {
		// not connected?
		if (!m_connected) {
			notConnected();
			return;
		...
		try {
			...
			if (m_serverVersion >= MIN_SERVER_VER_LINKING) {
				...
				int mktDepthOptionsCount = mktDepthOptions == null ? 0
						: mktDepthOptions.size();
	...
	public synchronized void placeOrder(int id, Contract contract, Order order) {
		...
		if (m_serverVersion < MIN_SERVER_VER_SCALE_ORDERS3) {
			if (order.m_scalePriceIncrement > 0
					&& order.m_scalePriceIncrement != Double.MAX_VALUE) {
		...
		if (m_serverVersion < MIN_SERVER_VER_SCALE_TABLE) {
			if (!IsEmpty(order.m_scaleTable)
					|| !IsEmpty(order.m_activeStartTime)
		...
		try {
			...
			if (m_serverVersion < MIN_SERVER_VER_ORDER_COMBO_LEGS_PRICE) {
				send(order.m_lmtPrice.equals(BigDecimal
						.valueOf(Double.MAX_VALUE)) ? new BigDecimal("0.0") : order.m_lmtPrice);
			} else {
				sendMax(order.m_lmtPrice);
			}
			if (m_serverVersion < MIN_SERVER_VER_TRAILING_PERCENT) {
				send(order.m_auxPrice == Double.MAX_VALUE ? 0
						: order.m_auxPrice);
			} else {
			...
			if (m_serverVersion >= MIN_SERVER_VER_SCALE_ORDERS3
					&& order.m_scalePriceIncrement > 0.0
					&& order.m_scalePriceIncrement != Double.MAX_VALUE) {
			...
			if (m_serverVersion >= MIN_SERVER_VER_LINKING) {
				...
				int orderMiscOptionsCount = orderMiscOptions == null ? 0
						: orderMiscOptions.size();
	...
	public synchronized boolean cancelOrder(int id) {
	...
	public synchronized void reqPositions() {
		...
		try {
			...
			error(EClientErrors.NO_VALID_ID,
					EClientErrors.FAIL_SEND_REQPOSITIONS, "" + e);
		}
	}
	...
	public synchronized void cancelPositions() {
		...
		try {
			...
			error(EClientErrors.NO_VALID_ID,
					EClientErrors.FAIL_SEND_CANPOSITIONS, "" + e);
		}
	}

	public synchronized void reqAccountSummary(int reqId, String group,
			String tags) {
		// not connected?
		if (!m_connected) {
			notConnected();
		...
		try {
			...
			error(EClientErrors.NO_VALID_ID,
					EClientErrors.FAIL_SEND_REQACCOUNTDATA, "" + e);
		}
	}
	...
	public synchronized void cancelAccountSummary(int reqId) {
		...
		try {
			...
			error(EClientErrors.NO_VALID_ID,
					EClientErrors.FAIL_SEND_CANACCOUNTDATA, "" + e);
		}
	}
	...
	public synchronized void verifyRequest(String apiName, String apiVersion) {
		...
		if (!m_extraAuth) {
			error(EClientErrors.NO_VALID_ID,
					EClientErrors.FAIL_SEND_VERIFYMESSAGE,
		...
		try {
			...
			error(EClientErrors.NO_VALID_ID,
					EClientErrors.FAIL_SEND_VERIFYREQUEST, "" + e);
		}
	}
	...
	public synchronized void verifyMessage(String apiData) {
		...
		try {
			...
			error(EClientErrors.NO_VALID_ID,
					EClientErrors.FAIL_SEND_VERIFYMESSAGE, "" + e);
		}
	}
	...
	public synchronized void queryDisplayGroups(int reqId) {
		...
		try {
			...
			error(EClientErrors.NO_VALID_ID,
					EClientErrors.FAIL_SEND_QUERYDISPLAYGROUPS, "" + e);
		}
	}
	...
	public synchronized void subscribeToGroupEvents(int reqId, int groupId) {
		...
		try {
			...
			error(EClientErrors.NO_VALID_ID,
					EClientErrors.FAIL_SEND_SUBSCRIBETOGROUPEVENTS, "" + e);
		}
	}
	...
	public synchronized void updateDisplayGroup(int reqId, String contractInfo) {
		...
		try {
			...
			error(EClientErrors.NO_VALID_ID,
					EClientErrors.FAIL_SEND_UPDATEDISPLAYGROUP, "" + e);
		}
	}
	...
	public synchronized void unsubscribeFromGroupEvents(int reqId) {
		...
		try {
			...
			error(EClientErrors.NO_VALID_ID,
					EClientErrors.FAIL_SEND_UNSUBSCRIBEFROMGROUPEVENTS, "" + e);
		}
	}
	...
	protected void send(BigDecimal val) throws IOException {
		send(val.toString());
	}
	...
	private void sendMax(BigDecimal val) throws IOException {
		if (val.equals(BigDecimal.valueOf(Double.MAX_VALUE))) {
			sendEOL();
		} else {
			send(val.toString());
		}
