...
public class EReader extends Thread {
	...
	protected boolean processMsg(int msgId) throws IOException, SQLException {
		...
		case OPEN_ORDER: {
			...
			if (version < 29) {
				order.m_lmtPrice = readBigDecimal();
			} else {
				order.m_lmtPrice = readBigDecimalMax();
			}
			if (version < 30) {
	...
	protected BigDecimal readBigDecimal() throws IOException {
		String str = readStr();
		return str == null ? new BigDecimal("0.0") : new BigDecimal(str);
	}
	...
	protected BigDecimal readBigDecimalMax() throws IOException {
		String str = readStr();
		return (str == null || str.length() == 0) ? BigDecimal
				.valueOf(Double.MAX_VALUE) : new BigDecimal(str);
	}
