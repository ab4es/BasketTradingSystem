...
public class EWrapperMsgGenerator extends AnyWrapperMsgGenerator {
	...
	static public String openOrder(int orderId, Contract contract, Order order,
		...
		String msg = "open order: orderId=" + orderId + " action="
				...
				+ " conid=" + contract.m_conId + " symbol=" + contract.m_symbol
				+ " secType=" + contract.m_secType + " expiry="
				+ contract.m_expiry + " strike=" + contract.m_strike
				+ " right=" + contract.m_right + " multiplier="
				+ contract.m_multiplier + " exchange=" + contract.m_exchange
				+ " primaryExch=" + contract.m_primaryExch + " currency="
				+ contract.m_currency + " localSymbol="
				+ contract.m_localSymbol + " tradingClass="
				+ contract.m_tradingClass + " type=" + order.m_orderType
				+ " lmtPrice=" + Util.BigDecimalMaxString(order.m_lmtPrice)
				+ " auxPrice=" + Util.DoubleMaxString(order.m_auxPrice)
				+ " TIF=" + order.m_tif + " localSymbol="
				+ contract.m_localSymbol + " client Id=" + order.m_clientId
				+ " parent Id=" + order.m_parentId + " permId="
				+ order.m_permId + " outsideRth=" + order.m_outsideRth
				+ " hidden=" + order.m_hidden + " discretionaryAmt="
				+ order.m_discretionaryAmt + " displaySize="
				+ order.m_displaySize + " triggerMethod="
				...
				+ Util.DoubleMaxString(order.m_deltaNeutralAuxPrice)
				...
				+ Util.DoubleMaxString(order.m_trailStopPrice)
				+ " trailingPercent="
				+ Util.DoubleMaxString(order.m_trailingPercent)
				+ " scaleInitLevelSize="
				...
				+ Util.DoubleMaxString(order.m_scalePriceAdjustValue)
				+ " scalePriceAdjustInterval="
				+ Util.IntMaxString(order.m_scalePriceAdjustInterval)
				+ " scaleProfitOffset="
				+ Util.DoubleMaxString(order.m_scaleProfitOffset)
				+ " scaleAutoReset=" + order.m_scaleAutoReset
				+ " scaleInitPosition="
				+ Util.IntMaxString(order.m_scaleInitPosition)
				+ " scaleInitFillQty="
				+ Util.IntMaxString(order.m_scaleInitFillQty)
		...
		if ("BAG".equals(contract.m_secType)) {
			...
			if (contract.m_comboLegs != null) {
				for (int i = 0; i < contract.m_comboLegs.size(); ++i) {
					...
					msg += " designatedLocation="
							+ comboLeg.m_designatedLocation;
					msg += " exemptCode=" + comboLeg.m_exemptCode;
					if (order.m_orderComboLegs != null
							&& contract.m_comboLegs.size() == order.m_orderComboLegs
									.size()) {
						OrderComboLeg orderComboLeg = order.m_orderComboLegs
								.get(i);
						msg += " price="
								+ Util.DoubleMaxString(orderComboLeg.m_price);
					}
					msg += ";";
				}
			}
			...
			if (order.m_basisPoints != Double.MAX_VALUE) {
				msg += " basisPoints="
						+ Util.DoubleMaxString(order.m_basisPoints);
				msg += " basisPointsType="
						+ Util.IntMaxString(order.m_basisPointsType);
			}
		}
	...
	static public String contractDetailsSecIdList(
			ContractDetails contractDetails) {
	...
	static public String scannerData(int reqId, int rank,
		...
		return "id = " + reqId + " rank=" + rank + " symbol="
				...
				+ contract.m_tradingClass + " distance=" + distance
	...
	static public String commissionReport(CommissionReport commissionReport) {
		String msg = "commission report:" + " execId="
				+ commissionReport.m_execId + " commission="
				+ Util.DoubleMaxString(commissionReport.m_commission)
				+ " currency=" + commissionReport.m_currency + " realizedPNL="
				+ Util.DoubleMaxString(commissionReport.m_realizedPNL)
	...
	static public String position(String account, Contract contract,
			int position, double avgCost) {
		String msg = " ---- Position begin ----\n" + "account = " + account
				+ "\n" + contractMsg(contract) + "position = "
				+ Util.IntMaxString(position) + "\n" + "avgCost = "
	...
	static public String accountSummary(int reqId, String account, String tag,
