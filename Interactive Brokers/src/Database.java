...
public class Database {
	...
	public static void addTransaction(String currency, String exchange,
		...
		int contractId = addContract(currency, exchange, primaryExch, secType,
				symbol);
		addOrder(contractId, action, lmtPrice, orderType, totalQuantity, tif,
	 ...
	 * 
	 * @Return m_contractId This is used as the foreign key for all corresponding
	...
	public static int addContract(String currency, String exchange,
			...
			throws SQLException {
		String insertQuery = "INSERT INTO `contracts` (`m_contractId`, `m_currency`, `m_exchange`, `m_primaryExch`, `m_secType`, `m_symbol`, `m_bid`, `m_ask`, `m_bidSize`, `m_askSize`, `m_lastPrice`)";
		insertQuery += " VALUES(NULL, '" + currency + "', '" + exchange
				+ "', '" + primaryExch + "', '" + secType + "', '" + symbol
				+ "', 0.0, 0.0, 0.0, 0, 0);";
		String m_contractIdQuery = "SELECT `m_contractId` FROM `contracts` WHERE `m_symbol` = '"
				+ symbol + "';";

		// Used to determine if the contract being added already exists
		int existingContractm_contractId = findm_contractId(symbol);

		// If the contract does not exist in the contracts table yet
		if (existingContractm_contractId == -1) {
			int m_contractId = -1;
			connection.createStatement().executeUpdate(insertQuery);
			ResultSet resultSet = connection.createStatement().executeQuery(
					m_contractIdQuery);
			resultSet.next();
			m_contractId = resultSet.getInt(1);
			System.out.print(getTimestamp() + "ADDED: ");
			printContract(m_contractId);
			return m_contractId;
		}
		// If the contract already exists in the contracts, use the existing
		// contract's m_contractId for the corresponding order's foreign key
		else {
			System.out.print(getTimestamp() + "NOT ADDED: (DUPLICATE) ");
			printContract(existingContractm_contractId);
			return existingContractm_contractId;
		}
	 ...
	 */
	public static void addOrder(int contractId, String action, double lmtPrice,
		...
		String query = "INSERT INTO `orders` (`m_orderId`, `m_contractId`, `m_action`, `m_lmtPrice`, `m_orderType`, `m_totalQuantity`, `m_tif`, `m_faGroup`, `m_faMethod`, `m_faProfile`, `m_account`, `m_orderRef`, `m_outsideRth`)";
		query += " VALUES (NULL, '" + contractId + "', '" + action + "', '"
				+ lmtPrice + "', '" + orderType + "', '" + totalQuantity
				+ "', '" + tif + "', ";
		String m_orderIdQuery = "SELECT `m_orderId` FROM `orders` ORDER BY `m_orderId` DESC LIMIT 1;";

		// Used to determine if the contract being added already exists
		int existingOrderm_orderId = findm_orderId(contractId, action, lmtPrice,
	 ...
	 * contractId is being used to find the appropriate contract as it is the value
	 * that used to request such market data.
	 */
	public static void updateBidAskLastPrice(int contractId, int field, double price)
			throws SQLException {
		String query = "";
		if (field == 1) {
			query = "UPDATE `contracts` SET `m_bid` = " + price
					+ " WHERE `m_contractId` = " + contractId + ";";
			connection.createStatement().executeUpdate(query);
		} else if (field == 2) {
	...
	/*
	 * Update the Bid and/or Ask sizes. The contractId is being used to find the
	 * appropriate contract as it is the value that is used to request such
	 * market data.
	 */
	public static void updateBidAskSizes(int contractId, int field, int size)
			throws SQLException {
		String query = "";
		if (field == 0) {
	...
	public static void updateOrderStatus(int orderId, int remaining)
		...
		if (getOrder(orderId) != null) {
			...
			Contract contract = getContract(order.m_contractId);
			if (remaining != 0) {
			...
			// If there are no more shares outstanding
				...
				int linkedOrdersSize = findLinkedOrders(order.m_contractId).size();
				...
				if (linkedOrdersSize <= 0) {
					...
					System.out.println();
					Database.deleteContract(order.m_contractId);
				} else {
	...
	public static void deleteContract(int contractId) throws SQLException {
		...
		String orderMsg = "";
		String toBeDeletedQuery = "SELECT `m_orderId` FROM `orders` WHERE `m_contractId` = "
				+ contractId + ";";
		ResultSet resultSet = connection.createStatement().executeQuery(
				toBeDeletedQuery);
		int counter = 0;
		...
		if (orderMsg.isEmpty()) {
			orderMsg += "no order(s) with m_contractId = " + contractId + "!";
		}

		// Delete orders linked to this contract first
		String orderDeletionQuery = "DELETE FROM `orders` WHERE `m_contractId` = "
				+ contractId + ";";
		connection.createStatement().executeUpdate(orderDeletionQuery);

		// Now delete the intended contract
		String contractDeletionQuery = "DELETE FROM `contracts` WHERE `m_contractId` = "
				+ contractId + ";";
	...
	/*
	 * @Return m_contractId if the contract already exists in the contracts table
	 * 
	 * @Return -1 if the contract does not exist
	 */
	public static int findm_contractId(String symbol) throws SQLException {
		String query = "SELECT `m_contractId` FROM `contracts` WHERE `m_symbol` = '"
				+ symbol + "';";
	 ...
	 */
	public static int findm_orderId(int contractId, String action, double lmtPrice,
			String orderType, int totalQuantity, String tif, String faGroup,
			String faMethod, String faProfile, String account, String orderRef,
			boolean outsideRth) throws SQLException {
		String query = "SELECT `m_orderId` FROM `orders` WHERE `m_contractId` = "
				+ contractId + " AND `m_action` = '" + action
	 ...
	 * contractId
	 */
	public static ArrayList<Order> findLinkedOrders(int contractId)
			throws SQLException {
		ArrayList<Order> orders = new ArrayList<Order>();
		String query = "SELECT * FROM `orders` WHERE `m_contractId` = " + contractId
				+ ";";
		ResultSet resultSet = connection.createStatement().executeQuery(query);
		...
		System.out.println("NO ORDERS WITH m_contractId = " + contractId);
		System.out.println();
		return null;
	}
	 ...
	 * Console outputs
	 */
	public static void printContract(int contractId) throws SQLException {
		String query = "SELECT * FROM `contracts` WHERE `m_contractId` = " + contractId
				+ ";";
		ResultSet resultSet = connection.createStatement().executeQuery(query);
		// contract was found with this contractId
		if (resultSet.next()) {
			System.out.println("contract[m_symbol="
					+ resultSet.getString("m_symbol") + "; m_contractId="
					+ resultSet.getInt("m_contractId") + "; m_currency="
		...
		// no contract was found with this contractId
		else
			System.out.println("no contract found with m_contractId = " + contractId
					+ "!");
	}

	public static String contractPrintMsg(int contractId) throws SQLException {
		String msg = "";
		String query = "SELECT * FROM `contracts` WHERE `m_contractId` = " + contractId
				+ ";";
		ResultSet resultSet = connection.createStatement().executeQuery(query);
		...
		if (resultSet.next()) {
			msg = "contract[m_symbol=" + resultSet.getString("m_symbol")
					+ "; m_contractId=" + resultSet.getInt("m_contractId")
		...
		// no contract was found with this contractId
			...
			msg = "no contract found with m_contractId = " + contractId + "!";
		return msg + "\n";
	}
	...
	public static void printOrder(int orderId) throws SQLException {
		...
		if (resultSet.next()) {
			System.out.println("order[m_orderId="
					+ resultSet.getInt("m_orderId") + "; m_contractId="
					+ resultSet.getInt("m_contractId") + "; m_action="
	...
	public static ResultSet getContractResultSet(int contractId) throws SQLException {
		String query = "SELECT * FROM `contracts` WHERE `m_contractId` = " + contractId
				+ ";";
		...
		if (resultSet.isBeforeFirst())
			return resultSet;
		System.out.println("No contract result set found with m_contractId = "
				+ contractId);
		System.out.println();
		return null;
	}
	...
	public static Contract getContract(int contractId) throws SQLException {
		Contract contract = new Contract();
		ResultSet resultSet = getContractResultSet(contractId);
		if (resultSet != null) {
			resultSet.next();
			contract.m_contractId = resultSet.getInt("m_contractId");
		...
		System.out
				.println("Null contract returned because no result set found with m_contractId = "
						+ contractId);
		System.out.println();
		return null;
	}
	...
	public static Contract getContractWithOrderId(int orderId)
			throws SQLException {
		String query = "SELECT `m_contractId` FROM `orders` WHERE `m_orderId` = "
				+ orderId + ";";
		ResultSet resultSet = connection.createStatement().executeQuery(query);
		resultSet.next();
		return getContract(resultSet.getInt("m_contractId"));
	}
	...
	public static Order getOrder(int orderId) throws SQLException {
		...
		if (resultSet != null) {
			...
			order.m_contractId = resultSet.getInt("m_contractId");
			order.m_action = resultSet.getString("m_action");
			ResultSet currencyResultSet = getContractResultSet(order.m_contractId);
	...
	public static ArrayList<Contract> getAllContracts() throws SQLException {
		...
		String query = "SELECT `m_contractId` FROM `contracts`;";
		ResultSet resultSet = connection.createStatement().executeQuery(query);
		if (resultSet.isBeforeFirst()) {
			while (resultSet.next()) {
				contracts.add(getContract(resultSet.getInt("m_contractId")));
			}
			return contracts;
		}
	...
	public static double getBid(int contractId) throws SQLException {
		String query = "SELECT `m_bid` FROM `contracts` WHERE `m_contractId` = "
				+ contractId + ";";
		ResultSet resultSet = connection.createStatement().executeQuery(query);
		resultSet.next();
	...
	public static double getAsk(int contractId) throws SQLException {
		String query = "SELECT `m_ask` FROM `contracts` WHERE `m_contractId` = "
				+ contractId + ";";
		ResultSet resultSet = connection.createStatement().executeQuery(query);
		resultSet.next();
	...
	public static double getLastPrice(int contractId) throws SQLException {
		String query = "SELECT `m_lastPrice` FROM `contracts` WHERE `m_contractId` = "
				+ contractId + ";";
		ResultSet resultSet = connection.createStatement().executeQuery(query);
		resultSet.next();
	...
	public static int getBidSize(int contractId) throws SQLException {
		String query = "SELECT `m_bidSize` FROM `contracts` WHERE `m_contractId` = "
				+ contractId + ";";
		ResultSet resultSet = connection.createStatement().executeQuery(query);
		resultSet.next();
	...
	public static int getAskSize(int contractId) throws SQLException {
		String query = "SELECT `m_askSize` FROM `contracts` WHERE `m_contractId` = "
				+ contractId;
		ResultSet resultSet = connection.createStatement().executeQuery(query);
		resultSet.next();
